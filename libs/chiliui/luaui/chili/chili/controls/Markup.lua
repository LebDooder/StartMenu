...
function Markup:New(obj)
  ...
  local html_table = VFS.Include(HTML_DIR..'html.lua')(obj.html or HTML_FILE)
  obj:Parse(html_table)
  return obj
end
...
function Markup:Parse(root, parent)
  ...
  for k, v in pairs(root) do
    ...
    if self:type(k,v) == 'element' then
      if v._tag:lower() =='style' then self:AddStyles(v[1]) return end
